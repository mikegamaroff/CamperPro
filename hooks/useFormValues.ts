...
export type SetValuesFunction<T> = (
...
export type SetValuesCallback<T> = (
  value: FormValuesType<T>
) => FormValueType<T>;
/**
...
export const useFormValues = <T>(
  ...
  const [formValues, setFormValues] = useState<FormValuesType<T> | undefined>(
    undefined
  );
  const [stateDataObject, setStateDataObject] = useState<T | null | undefined>(
    null
  );
  ...
  useEffect(
    () =>
      setStateDataObject((oldDataObject) => {
        ...
        if (!dataIsSameObject) {
          const totalFieldsArray = Object.keys(
            dataObject || formConfig
          ) as (keyof T)[];
  ...
  const validateForm = (): boolean => {
    if (formValues) {
      let totalFieldsArray = Object.keys(formValues) as (keyof T)[];
      let blankValuesContainer: FormValuesType<T> = {};

      totalFieldsArray.forEach((fieldName) => {
        const { errors } = validate(
          formConfig[fieldName] as FormConfigType,
          formValues[fieldName]?.value as any
        );
        const val: FormValuesType<T> = {
          [fieldName]: {
